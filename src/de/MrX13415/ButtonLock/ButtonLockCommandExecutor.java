...
//							  		currentPlayerVars.getPlayer().sendMessage(Language.TEXT_SUCCEED);
	  		    		 ...
	  		    		 if (args.length == 3){
	 		  				...
	 		  				if (args[1].equalsIgnoreCase("forcePassword") || args[1].equalsIgnoreCase("forcepw") || args[1].equalsIgnoreCase("fpw")) {
	 	  						if (group.isUnlocked()) {
	 	  							boolean setting = Boolean.parseBoolean(args[2]);
	 			  				
	 		  						boolean settingBefore = group.isForceingEnterPasswordEveryTime();
	 		  						if (setting != settingBefore) group.changedSetting_forceEnterPasswordEveryTime = true;
	 		  				...
	 		  				if (args[1].equalsIgnoreCase("costs") || args[1].equalsIgnoreCase("cost") || args[1].equalsIgnoreCase("c")) {
	 	  						if (group.isUnlocked()) {
	 		  						double setting = Double.valueOf(args[2]);
